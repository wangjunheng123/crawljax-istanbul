...
public class Crawler {
    ...
    public static void getCodeCoverageStatus() {
        try {
            LOG.info("now start to get code coverage");
    ...
    public static void updateCodeCoverage(Object coverageObj) {
        try {
            LOG.info("start to update code coverage");
            String url = "http://localhost:6969/coverage/client";
            Gson gson = new Gson();
            String jsonString = gson.toJson(coverageObj);
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()))) {
                ...
                LOG.info("HTTP Response: " + response.toString());
            }
    ...
    private void crawlThroughActionsOld() {
        ...
        while (action != null && !interrupted) {
            Object coverageObj = browser.executeJavaScript("return window.__coverage__");
            Crawler.updateCodeCoverage(coverageObj);
            if(count % 5 ==0){
                getCodeCoverageStatus();
            }
            count++;
